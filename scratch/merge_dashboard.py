...
# Extract script block from current templates/dashboard.html
with open("templates/dashboard.html", "r", encoding="utf-8") as f:
    old_dashboard = f.read()

script_match = re.search(r"<script>(.*?)</script>", old_dashboard, re.DOTALL)
if not script_match:
    print("Error: Could not extract JavaScript from old dashboard.")
    exit(1)
old_js = script_match.group(1)

# Perform layout adjustments to accommodate dynamic controls and widgets
# 1. Add {% load static %} at the very top
layout = "{% load static %}\n" + layout

# 2. Inject manifest and Chart.js into head
head_inject = """
...
# 3. Add dynamic variables in head styles
head_styles = """
        .gauge-track {
            stroke: #1e2023;
...
# 4. Integrate TopNavBar components
...
# Replace top navbar status widget
...
<div id="sensorPill" class="flex items-center gap-2 bg-[#1e2023] px-3 py-1 rounded-full text-xs font-semibold text-outline-variant">
    <div id="sensorDot" class="w-2 h-2 rounded-full bg-outline"></div>
...
# 5. Integrate Left Sidebar components (Device Selector, Add Device button)
...
# 6. Overview Navigation Active State link
...
# 7. Main Dashboard Content Header (Title + Low Alert Badge)
header_old = """<div>
...
# 8. Bento Grid Card A: Circular Gauge & Info Details
...
# 9. Bento Grid Card B: Motor Control Panel Override
        ...
        <div class="space-y-4">
            ...
            <div id="manualControlsBlock" class="space-y-2">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button id="btnOn" onclick="motorOn()" class="py-3 bg-outline-variant/30 text-outline rounded-lg text-xs font-bold uppercase tracking-wider transition-all">START</button>
                    <button id="btnOff" onclick="motorOff()" class="py-3 bg-outline-variant/30 text-outline rounded-lg text-xs font-bold uppercase tracking-wider transition-all">STOP</button>
                </div>
            </div>
        </div>
    </div>
...
# 10. Bento Grid Card C: Power Node override
...
# 11. Bento Grid Card D: Historical Trends Chart
...
# Inject System Configurations Card, Configurations Display Card, Logs table, Add Device Modal Overlay, Audio buzzers, and the Script block
bento_grid_end = '</div>\n</main>'
...
# Inject dynamic overlay modal for BLE and claim setups
modal_html = """
...
# Replace navigation bar (Mobile Only) at the very bottom
...
# Inject modal and buzzers
layout = layout.replace("</body>", modal_html + "\n</body>")

# 12. Tweak and merge JS script updates
# Let's adjust details inside old_js to hook cleanly into the new DOM:

# Tweak isDark() and toggleDark() functions
old_js = old_js.replace(
...
# Tweak polling calculations segment to update the circular SVG gauge, the voltage level, and the bento metrics
...
old_js = old_js.replace(calculated_poll_old, calculated_poll_new)

# Tweak Alarm alert badge layout updater
alerts_old = """            /* ---------- ALERTS ---------- */
...
old_js = old_js.replace(alerts_old, alerts_new)

# Tweak motor state polling class assignments for manual/auto buttons in Tailwind
...
old_js = old_js.replace(motor_state_poll_old, motor_state_poll_new)

# Tweak Chart filters active states in toggleDataset and switchTimeRange
...
old_js = old_js.replace(chart_filters_old, chart_filters_new)
...
old_js = old_js.replace(toggle_dataset_old, toggle_dataset_new)

# Inject corrected javascript script content back
layout = layout.replace("<script></script>", f"<script>\n{old_js}\n</script>")

# Write the final file
with open("templates/dashboard.html", "w", encoding="utf-8") as f:
    f.write(layout)

print("Dashboard design overhauled and successfully merged!")
